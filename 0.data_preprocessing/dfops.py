...
class DfOps:
    ...
    @staticmethod
    def import_data_from_csv(fp: PurePath, sep: str = ",", low_memory: bool = True, verbose: bool = True, nrows=None):
        df = pd.read_csv(fp, sep=sep, low_memory=low_memory, verbose=verbose, nrows=nrows)
        if isinstance(df, pd.DataFrame):
            return df
        else:
            raise Exception(f"No dataframe was created when trying pd.read_csv({fp})")
    ...
    # date format: https://www.dataindependent.com/pandas/pandas-to-datetime/
    ...
    def convert_string_to_date(self, date_string: str, date_format):
        datetime_object = datetime.strptime(date_string, date_format)
        pp(datetime_object)

    # eg r'\r+|\n+|\t+'
    def replace_via_regex_in_df(self, regex_find, replace=''):
        self.df = self.df.replace(regex_find, replace, regex=True)
